...
class Node extends EventEmitter {
  ...
  printIpAddressesStatistics() {
    ...
    console.log(`from\t\trequest\tchallenge\tanswer\tverification\tfailed\tsucceeded`)
    for (const ipAddress of sortedIpAddresses) {
      const statistics = this.ipAddressesStatistics[ipAddress].getStatistics()
      console.log(`${pubSubNetwork.nodesIpAddresses[ipAddress].name}\t${statistics.challengeRequest || 0}\t${statistics.challenge || 0}\t\t${statistics.challengeAnswer || 0}\t${statistics.challengeVerification || 0}\t\t${statistics.incompleteChallengeCount || 0}\t${statistics.completeChallengeCount || 0}`)
    }
  }
...
class CommunityOwnerNode extends Node {
  async handleMessage(message) {
    ...
    // community owner node can deliver challenges to those who request it
    if (message.type === 'challengeRequest') {
      this.sendChallenge(message.value)
    }

    // community owner node can validate challenge answers
...
class IpAddressStatistics {
  ...
  addMessage(message, {firstSeen} = {}) {
    ...
    // never consider "challenge" incomplete because it comes from community owner, we know it's not spam
...
let friendlyNodesMessageCount = 10
let spammerNodeMessageCount = 90

while (friendlyNodesMessageCount--) {
  friendlyNode2.sendChallengeRequest()
  friendlyNode3.sendChallengeRequest()
}
while (spammerNodeMessageCount--) {
